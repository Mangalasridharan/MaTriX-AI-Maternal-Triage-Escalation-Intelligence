...
from app.db.schemas import CaseSubmission, CaseResult, HistoryItem
...
class SignupRequest(BaseModel):
    username: str
    password: str
    clinic_name: str = "Default Clinic"


@router.post("/auth/signup", tags=["Auth"], summary="Create a new clinic account")
async def signup(payload: SignupRequest, db: AsyncSession = Depends(get_db)):
    existing = await get_user_by_username(db, payload.username)
    if existing:
        raise HTTPException(status_code=409, detail="Username already exists")
    await create_user(db, payload.username, payload.password, payload.clinic_name)
    return {"message": "Account created", "username": payload.username}


@router.post("/auth/token", tags=["Auth"], summary="Obtain JWT access token")
